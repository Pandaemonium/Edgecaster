...
def _game(zone_w: int = 10, zone_h: int = 10) -> object:
    idx = SpatialIndex(bin_size=4)
    game = SimpleNamespace(
        spatial_index=idx,
        cfg=SimpleNamespace(world_width=zone_w, world_height=zone_h),
    )
    return game
...
def test_large_entity_is_queryable_away_from_center_zone() -> None:
    ...
    register_proxy_entity(game, ent, zone_coord=(5, 5, 0), local_pos=(0, 0))

    # Center is at (50,50), size=60 means footprint ~(20,20,80,80). Query inside that footprint.
    hits = game.spatial_index.query_rect((22.0, 24.0, 24.0, 26.0), 0)
...
def test_query_deduplicates_entities_indexed_into_multiple_zone_cells() -> None:
    ...
    entries = game.spatial_index.query_rect((0.0, 0.0, 100.0, 100.0), 0)
...
def test_explicit_footprint_abs_takes_precedence_over_abs_size() -> None:
    ...
    inside = game.spatial_index.query_rect((12.0, 10.0, 13.0, 11.0), 0)
    inside_ids = {str(getattr(e.obj, "id", "")) for e in inside}
    assert "river_a" in inside_ids

    outside = game.spatial_index.query_rect((49.0, 49.0, 51.0, 51.0), 0)
