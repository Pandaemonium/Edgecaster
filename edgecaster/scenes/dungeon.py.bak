...
class DungeonScene(Scene):
    ...
    def run(self, manager: "SceneManager") -> None:  # type: ignore[name-defined]
        ...
        # Prepare renderer for dungeon run (target cursor, abilities, etc.)
            ...
            while running:
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        # Hard exit: clear all scenes
                        manager.set_scene(None)
                        running = False
                        break

                    elif event.type == pygame.KEYDOWN:
                        # All keyboard input goes through GameInput + _handle_command.
                        cmds = self.input.handle_keydown(event)
                        for cmd in cmds:
                            self._handle_command(game, renderer, cmd, manager)
                        # If no commands were produced, key is ignored.
                ...
                # ----------------- DRAW -----------------
                renderer.draw_dungeon_frame(game)
                clock.tick(60)

                # ----------------- EXIT CONDITIONS -----------------
                # If some input handler requested that we quit the dungeon loop
                # (e.g. to open inventory, pause, or show urgent popup), honor that here.
                if renderer.quit_requested or getattr(renderer, "pause_requested", False):
                    running = False
                    continue
    ...
    def _handle_command(
        ...
        if kind == "quick_activate_all":
            ...
            if renderer.aim_action == "activate_all":
                ...
                # we can reuse the renderer's hover update; mouse pos is fine as a heuristic
        ...
        # Epiphenomenal debug: body-hop into nearest other actor
        ...
        if kind == "open_inventory":
            if not in_aim_mode:
                ...
                # Ask the dungeon loop to yield so the scene manager can
                # push InventoryScene.
                renderer.quit_requested = True
            return
        ...
        # Stairs up OR world map on surface
        ...
        if kind == "move" and vec is not None:
            # At this point, we are *not* in terminus mode, dialog, or config,
            # and aim_mode only intercepts on confirm, not on movement.
        ...
        if kind == "confirm":
            # Not dialog/config/terminus/aim (those already returned above).
