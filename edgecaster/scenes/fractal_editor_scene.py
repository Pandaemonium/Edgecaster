...
class FractalEditorScene(Scene):
    ...
    - Left click empty: add vertex (snap to nearest grid intersection)
    - Left click vertex: select; if one already selected, create edge from selected -> clicked
    - Left click near edge: select edge
    - Right click vertex: delete vertex and incident edges
    ...
    def run(self, manager) -> None:  # type: ignore[override]
        ...
        def accept() -> None:
            pts = list(self.state.vertices)
            # ensure anchors at (0,0) -> (10,0) for correct scaling in CustomPolyGenerator
            if not pts or pts[0] != (0, 0):
                if (0, 0) not in pts:
                    pts.insert(0, (0, 0))
                else:
                    # move existing root to front
                    pts.remove((0, 0))
                    pts.insert(0, (0, 0))
            if pts[-1] != (10, 0):
                if (10, 0) not in pts:
    ...
    def _delete_vertex(self, idx: int) -> None:
        if idx < 0 or idx >= len(self.state.vertices):
            return
        # find first outgoing target to reattach incoming edges
        outgoing = [b for (a, b) in self.state.edges if a == idx]
        reattach_to: Optional[int] = outgoing[0] if outgoing else None
        new_edges: List[Tuple[int, int]] = []
        for a, b in self.state.edges:
            if a == idx:
                continue
            if b == idx:
                if reattach_to is not None and reattach_to != idx:
                    new_edges.append((a, reattach_to))
                continue
        ...
        for a, b in new_edges:
            na = a
            nb = b
            if isinstance(a, int) and a >= 0 and a > idx:
                na = a - 1
            if isinstance(b, int) and b >= 0 and b > idx:
                nb = b - 1
    ...
    def _draw(self, surface: pygame.Surface) -> None:
        ...
        if self._small_font:
            lines = [
                f"Mode: {self.mode.upper()} (1 draw, 2 delete, 3 flip)",
    ...
    def _mode_button_rects(self, panel: pygame.Rect):
        ...
        right_x = panel.right - btn_w - 20
        top_y = 20
        buttons = [("draw", "Draw (1)"), ("delete", "Delete (2)"), ("flip", "Flip (3)")]
        rects = []
        y = top_y
